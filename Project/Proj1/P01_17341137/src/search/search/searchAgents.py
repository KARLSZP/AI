...
class CornersProblem(search.SearchProblem):
    ...
    """
    # A search state in this problem is 
    #   a tuple ( pacmanPosition, Corner ) where:
    # pacmanPosition: a tuple (x,y) of integers specifying 
    #                 Pacman's position.
    # Corner:         a list of each corner's state.

    # Successors in this problem is 
    #   a list of tuple( nextstate, action, cost ) where:
    ...
    def getStartState(self):
        ...
        space)
        """
        # corners to record if the food in the corner is eaten
        corners = [0, 0, 0, 0]
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            # No collision, check if the position is a corner
            if not hitsWall:
                # Go through 4 corners
                if (nextx, nexty) in self.corners:
                    for pos in range(len(self.corners)):
                        tmp_corners = [i for i in cur_corners]
                        if self.corners[pos] == (nextx, nexty):
                            tmp_corners[pos] = 1
                            succ = (((nextx, nexty), tmp_corners), action, 1)
                # Still On the way
                else:
                    succ = (((nextx, nexty), cur_corners), action, 1)
                # Add to successors
