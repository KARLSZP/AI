...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for layer in range(self.num_layers):
            ...
            self.params['W%d' % (layer+1)] = weight_scale * \
                np.random.randn(layer_dim[0], layer_dim[1])
    ...
    def loss(self, X, y=None):
        ...
        for layer in range(self.num_layers):
            # forward
            Wi, bi = self.params['W%d' %
                                 (layer+1)], self.params['b%d' % (layer+1)]
            ...
            if self.use_batchnorm and layer != self.num_layers-1:
                gammai, betai = self.params['gamma%d' % (
                    layer + 1)], self.params['beta%d' % (layer + 1)]
                outi, bn_cachei = batchnorm_forward(
                    outi, gammai, betai, self.bn_params[layer])
            ...
            # dropout
            if self.use_dropout:
                outi, dropout_cachei = dropout_forward(
                    outi, self.dropout_param)
