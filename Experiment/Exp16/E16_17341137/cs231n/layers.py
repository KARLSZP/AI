...
def batchnorm_backward(dout, cache):
    ...
    N, D = x.shape
    dx_ = dout*gamma
    d_running_var = np.sum(dx_*(x-running_mean), axis=0) * \
        (-0.5)*(running_var+eps)**(-3.0/2.0)
    d_running_mean = np.sum(dx_*(-1.0/np.sqrt(running_var+eps)),
                            axis=0)+d_running_var*(np.sum(-2*(x-running_mean), axis=0)/N)
    dx = dx_*(1.0/np.sqrt(running_var+eps))+d_running_var * \
        2*(x-running_mean)/N+d_running_mean*1.0/N
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    H_, W_ = int(1 + (H + 2 * pad - HH) / stride), int(1 +
                                                       (W + 2 * pad - WW) / stride)
    ...
    for ni in range(x.shape[0]):
        for fi in range(w.shape[0]):
            for xi in range(H_):
                for yi in range(W_):
                    out[ni, fi, xi, yi] = np.sum(
                        x_pad[ni, :, xi * stride:xi * stride + HH, yi * stride:yi * stride + WW] * w[fi, :, :, :])
...
def conv_backward_naive(dout, cache):
    ...
    for ni in range(x.shape[0]):
        for fi in range(w.shape[0]):
            for xi in range(H_):
                for yi in range(W_):
                    dw[fi, :, :, :] += dout[ni, fi, xi, yi]*x_pad[ni, :, xi *
                                                                  stride:xi * stride + HH, yi * stride:yi * stride + WW]
                    dx_pad[ni, :, xi * stride:xi * stride + HH, yi * stride:yi *
                           stride + WW] += dout[ni, fi, xi, yi] * w[fi, :, :, :]
...
def max_pool_forward_naive(x, pool_param):
    ...
    H_, W_ = int(1 + (H - pool_height) / stride), int(1 +
                                                      (W - pool_width) / stride)
    ...
    for ni in range(x.shape[0]):
        for ci in range(x.shape[1]):
            for xi in range(H_):
                for yi in range(W_):
                    out[ni, ci, xi, yi] = np.max(
                        x[ni, ci, xi * stride:xi * stride + pool_height, yi * stride:yi * stride + pool_width])
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
...
def max_pool_backward_naive(dout, cache):
    ...
    for ni in range(x.shape[0]):
        for ci in range(x.shape[1]):
            for xi in range(H_):
                for yi in range(W_):
                    search_range = x[ni, ci, xi * stride:xi * stride +
